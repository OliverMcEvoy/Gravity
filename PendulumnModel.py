...
def derivs(state, t):
    dydx = np.zeros_like(state)  # create an array of zeros of the same shape as state
    dydx[0] = state[1]           # the derivative of the angle is tx    e angular velocity
    dydx[1] = -(g/L) * sin(state[0])  # the derivative of the angular velocity is the negative of (g/L)*sin(angle)
    return dydx
